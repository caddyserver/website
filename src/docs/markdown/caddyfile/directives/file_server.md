...
- **disable_canonical_uris** disables the default behaviour of redirecting to add a trailing slash if the request path is a directory, or remove the trailing slash if the request path is a file. Note that by default, canonicalization does not happen if the filename was underwent an internal rewrite (possibly via the `rewrite` directive), to avoid clobbering an explicit rewrite with implicit behaviour.
